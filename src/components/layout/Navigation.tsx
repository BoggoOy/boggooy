...
export default function Navigation() {
  ...
  return (
    <>
      ...
      {/* Mobile menu — outside nav to avoid mix-blend-difference */}
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            initial={{ opacity: 0 }}
            ...
            className="md:hidden fixed inset-0 bg-neutral-900 z-[100] flex flex-col items-center justify-center gap-8"
          >
            {navLinks.map((link) => (
              <Link
                key={link.href}
                ...
                className="text-white fluid-h2 font-light tracking-tight hover:opacity-70 transition-opacity"
              >
                {link.label}
              </Link>
            ))}
